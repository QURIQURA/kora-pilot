...
## 2026-09-26 — 버전 비교 표 재료 순서 드래그, 이제 저장됩니다

Component 상세 페이지의 "버전 비교" 표(여러 버전을 나란히 보는 엑셀 형태 표)에서 재료 행을 드래그로
재정렬해도 페이지를 벗어나면 초기화되던 문제를 고쳤습니다. 이제 드래그하면 화면에 보이는 모든
버전에 같은 순서로 바로 저장됩니다(각 버전에 실제로 있는 재료만 그 버전 안에서 순서가 맞춰집니다 —
예를 들어 V3에만 있는 재료는 V1/V2 순서에는 영향 없음). 열(버전) 순서 바꾸기나 칸 크기 조절은
지금처럼 이 화면을 보는 동안만 유지되는 "보기 편의" 기능으로 남아있습니다.

---

## 2026-09-24 — FORMULA 배수 열을 "몰드 + 개수" 기준으로 개편

지금까지 배수 열은 그냥 숫자(×2, ×3 등)였고, MOULD 선택과는 아무 연관이 없었습니다. 실제로 레시피를
볼 때는 "6인치 몇 개 / 8인치 몇 개"처럼 몰드 기준으로 생각하시는데, 화면은 추상적인 배수 숫자만
보여줘서 개념이 따로 놀았던 문제를 고쳤습니다.

- **+ ADD BATCH가 1단계로**: 누르면 바로 팝업이 뜨고, 몰드를 고르고 개수를 입력하면 그 값 그대로
  열이 만들어집니다. 이전처럼 일단 ×2로 만들고 나중에 따로 고치는 방식이 없어졌습니다.
- **열 제목이 "6인치 ×3개"처럼 표시**: 같은 몰드면 개수를 그대로 배수로 쓰고, 다른 몰드를 고르면
  두 몰드의 기준중량(MOULDS 설정에 등록된 g)을 비교해서 자동으로 배수를 환산합니다. (예: 8인치
  기준 레시피에서 6인치 열을 추가하면, 두 몰드 기준중량 비율만큼 자동으로 줄어든 양이 계산됩니다.)
- 기준중량이 등록 안 된 몰드끼리는 정확한 환산이 불가능하므로 개수를 그대로 배수로 쓰고 "⚠"
  표시가 뜹니다 — MOULDS 설정에서 기준중량(g)을 등록하면 바로 정확해집니다.
- 크림/가나슈 등 몰드 개념이 없는 BASE WEIGHT 방식 Component는 지금처럼 배수를 직접 숫자로
  입력하는 방식 그대로입니다. 몰드 방식 Component에서도 "배수 직접입력으로" 버튼으로 언제든
  전환할 수 있습니다.
- PRODUCTION 세션의 실제 계량 화면(WEIGHING MATRIX)은 이번 변경과 무관합니다 — 그쪽은 소수점
  배수가 실사용에서 불편하다는 이유로 예전에 자동계산 기능을 뺐던 전례가 있어 그대로 유지했고,
  이번 변경은 FORMULA 페이지의 레시피 검토/비교용 표에만 적용됩니다.

---

## 2026-09-24 — FORMULA 배수 열 미리보기 미반영 수정 (2차)

바로 전 수정(입력칸 표시)만으로는 부족했습니다 — 배수 숫자를 바꿔도 SAVE를 누르기 전까지 아래
재료량 칸이 계속 예전 배수(예: ×2) 기준으로 보여서 "적용이 안 된다"고 느껴지는 문제가 있었습니다.
이제 BASE ×1 재료량처럼 배수도 입력하는 즉시 재료량 칸에 미리 반영됩니다(저장은 여전히 SAVE를
눌러야 확정). 열 삭제 버튼도 영향 없습니다.

---

## 2026-09-24 — FORMULA "+ ADD BATCH" 새 열 표시 버그 수정

새로 "+ ADD BATCH"로 배수 열을 추가하면 기본값 ×2로 저장되는데, 화면에는 그 숫자가 안 보이고 빈
칸으로 표시되어 "이게 무슨 숫자 기준인지" 헷갈리는 문제가 있었습니다. 이제 새로 추가한 열도 바로
저장된 기본값(×2)이 입력칸에 표시됩니다 — 원하는 배수로 바로 수정하면 됩니다. (참고: 이 표는 항상
"BASE ×1" 열의 재료량 × 배수로 계산되고, 계산 자체는 이전에도 정확했습니다 — 화면 표시만의 문제였
습니다.)
...
**무엇이 달라졌나**
- PRODUCT 상세 화면 상단에 "제작방법" 선택칸이 추가됐습니다 — CATEGORY(제품군)와는 별개로, 파운드케익법/시폰법처럼 실제 공정이 같은 제품끼리 묶는 지표입니다.
- SETTINGS → TECHNIQUE 그룹에 "WORKFLOW TEMPLATES" 섹션이 새로 생겼습니다 — 제작방법마다 표준 TASK 순서(이름/TYPE/선행 관계, 시간은 당연히 없음)를 미리 등록해두고 ▲▼로 순서를 정리할 수 있습니다.
- PRODUCTION 작업 세션의 WORKFLOW 탭에 "템플릿 불러오기" 패널이 생겼습니다 — 제작방법과 템플릿을 고르고 "이 템플릿 적용"을 누르면, 등록해둔 TASK들이 순서/선행관계까지 그대로 세션에 깔립니다. 매번 같은 TASK를 손으로 다시 입력할 필요가 없어집니다.

**확인 방법**
- PRODUCT 상세에서 CATEGORY 옆에 "제작방법" 드롭다운이 보이는지 확인하세요.
- SETTINGS에서 제작방법을 하나 골라 템플릿 + TASK 몇 개를 등록해보세요.
- PRODUCTION 작업 세션의 WORKFLOW 탭에서 "템플릿 불러오기"를 펼쳐 방금 만든 템플릿을 적용해보고, TASK 순서/이름/선행관계가 그대로 들어오는지 확인하세요.

---

## 2026-09-23 — WORKFLOW "계획" 폐지 + TASK 순서(위/아래) 재정렬, BAKE 관찰값, PRODUCT COST 합계 표시

**무엇이 달라졌나**
- WORKFLOW TASK의 "계획(예정 시작/종료 시각)" 개념을 없앴습니다 — 실제 작업 시간과 STATUS가 안 맞아 혼란스럽다는 피드백을 반영했습니다. 이제 시간을 미리 정하지 않아도 TASK를 만들 수 있고, 타임라인은 오직 실제 시작/완료 시각만 기준으로 그립니다.
- 대신 아직 시작 전인 TASK는 "TASK 순서" 목록에 모여 ▲▼ 버튼으로 위/아래 순서를 바꿀 수 있습니다 — 시간 없이도 어떤 순서로 작업할지 미리 그려볼 수 있습니다. 이 순서는 TASK LIST에도 그대로 반영됩니다.
- 타임라인 자체는 그대로 유지됩니다 — 여러 품목이 동시에 진행될 때 실제 시간축 기준으로 무엇이 돌아가는지 보여주는 용도이며, 이번에 가로/세로 크기를 키워 텍스트와 박스가 안 잘리도록 가독성을 높였습니다.
- BAKE TASK에 "관찰값" 입력칸을 추가했습니다 — 상태(텍스트), 오븐투입시/중간/최종 높이(mm)를 기록할 수 있습니다. 나중에 다른 TASK TYPE에도 넓힐 수 있도록 만들었고, 이 데이터는 추후 따로 모아 시각화할 예정입니다.
- PRODUCT 상세의 COST BREAKDOWN 3열 그리드에서, 2열(COMPONENTS & ADJUSTMENT)과 3열(PRODUCTION COST) 각각 헤더 바로 아래(스크롤 안 되는 위치)에 합계를 고정 표시했습니다. 3열에는 "항목 합계 + OVERHEAD = 합계"를 함께 보여줘서, 1열 "총원가" 계산식의 PRODUCTION COST 금액이 왜 3열 리스트 합계보다 커 보였는지(OVERHEAD가 리스트엔 없고 합계에만 더해짐) 바로 확인할 수 있습니다.

**확인 방법**
- PRODUCTION 작업 세션의 WORKFLOW 탭에서 TASK를 추가할 때 시간 입력칸이 없는지, 시작 전 TASK가 "TASK 순서" 목록에서 ▲▼로 움직이는지 확인하세요.
- TASK TYPE을 BAKE로 입력하면 관찰값 입력칸이 나타나는지 확인하세요.
- PRODUCT 상세 페이지 COST BREAKDOWN에서 2열/3열 상단에 합계 줄이 보이는지, 1열 계산식과 맞는지 확인하세요.

---

## 2026-09-24 — PRODUCTION 배수(MULTIPLIER) 계산, 몰드/기준중량 역산 대신 개수 그대로 사용

**무엇이 달라졌나**
- 지금까지 PRODUCTION 작업 세션에 배합을 추가할 때 "몰드 기준 반죽량 × 개수 ÷ BASE 레시피 총량"으로 배수를 역산해서 자동으로 채워줬는데, 이 계산이 소수점(예: ×7.94)으로 떨어지는 경우가 많아 저울로 계량하기 불편하다는 피드백을 반영해 이 기능을 없앴습니다.
- 이제 "개수(배수) ×N"에 만들 개수(예: 4)를 그대로 입력하면 그게 배수가 됩니다 — 몰드/기준중량 선택, QTY 입력, 자동 계산 안내 문구가 모두 사라졌습니다.
- 배수 변경 이력(HISTORY)은 그대로 남아있습니다.

**확인 방법**
- PRODUCTION 작업 세션에서 배합을 새로 추가하거나 기존 배합의 배수를 수정할 때, MOULD/BASE WEIGHT 선택 없이 "개수(배수) ×N" 칸만 남아있는지 확인하세요.
...
**무엇이 달라졌나**
- TASK 블록이 시간대를 통째로 채우는 박스 방식에서, 시작 시각/종료 시각 마커 두 줄을 얇은 색 연결선(rail)으로 이은 방식으로 바뀌었습니다.
- 같은 품목(열) 안에서 TASK 시간대가 겹치면(예: 긴 TASK 도중 짧은 TASK가 시작) 각 TASK가 서로 다른 레인에 자기만의 연결선을 그리므로, 어느 선이 어느 TASK인지 항상 구분됩니다 — 이전 박스 방식은 겹치는 TASK가 같은 자리를 그대로 덮어써서 구분이 안 됐습니다.
- 시작/종료 시각이 같은 순간에 몰리는 경우(예: 여러 TASK가 같은 시각에 시작)에도 마커 폭을 자동으로 나눠서 겹치지 않게 표시합니다.

**확인 방법**
- PRODUCTION 작업 세션의 WORKFLOW 탭에서, 시간대가 겹치는 TASK 여러 개를 만들어보고 각각의 시작/종료 라벨과 연결선이 서로 구분되는지 확인하세요.
...
**무엇이 달라졌나**
- 계획 종료 시각(끝나는 시간)을 안 채운 TASK는 타임라인에 아예 그려지지 않던 버그를 고쳤습니다. 현장에서 시작 시각만 찍고 종료 시각을 안 채운 경우가 많아서, 방금 넣은 TASK TYPE 색상 박스가 "여전히 하나도 안 보인다"는 문제로 이어졌습니다.
- 이제 계획 시작 또는 실제 시작 시각 중 하나만 있어도 타임라인에 표시됩니다. 종료 시각은 실제 완료 → 계획 종료 → (진행 중이면) 지금 시각 → 그것도 없으면 화면에 보이기 위한 임시 30분 폭 순서로 정해집니다(저장되는 값이 아니라 화면 표시용입니다).

**확인 방법**
- WORKFLOW 탭에서 계획 종료 시각 없이 시작만 찍은 TASK가 타임라인에 색상 박스로 나타나는지 확인.

---

## 2026-09-24 — WORKFLOW TASK 여러 개를 하나의 다음 단계로 묶는 "선행 TASK" 기능

**무엇이 달라졌나**
- TASK를 추가하거나 수정할 때 "선행 TASK"를 여러 개 선택할 수 있습니다. 예를 들어 Yolk mixture와 Meringue를 둘 다 선행 TASK로 골라서 "Fold & Bake" 같은 다음 단계를 만들면, TASK 목록에 "이전 단계: Yolk mixture + Meringue"라고 표시됩니다.
- 순서 표시용 목적 외엔 쓰지 않던 기존 필드와 별개로 새로 만든 구조라, 기존 TASK 데이터에는 영향이 없습니다.

**확인 방법**
- WORKFLOW 탭에서 TASK를 추가/수정할 때 "선행 TASK" 체크박스로 이전 단계를 여러 개 골라보고, TASK 목록에 "이전 단계: ..."로 표시되는지 확인.

---

## 2026-09-24 — WORKFLOW 타임라인 TASK 블록을 TASK TYPE 색으로 꽉 채우도록 재조정

**무엇이 달라졌나**
- 방금 추가한 "TASK TYPE 색상 설정"이 실제 타임라인에는 반영되지 않던 문제를 고쳤습니다. 이제 각 TASK 블록 전체가 그 TASK TYPE의 색으로 채워진 박스로 그려지고, 참고해주신 planner 앱처럼 해당 시간대를 박스가 차지하는 형태로 보입니다.
- 소요 시간이 짧은 TASK도 박스 안에 시작 시각·TASK 이름·TYPE·종료 시각이 다 보이도록 최소 높이는 그대로 유지됩니다.

**확인 방법**
- PRODUCTION → WORKFLOW 탭에서 "TASK TYPE 색상 설정"으로 원하는 색을 고른 뒤, 타임라인의 해당 TASK 블록이 그 색으로 꽉 채워지는지 확인.

---

## 2026-09-24 — PRODUCT COMPONENTS 사이즈 행 정리 자동화 + 예상원가 표시 강조

**무엇이 달라졌나**
- PRODUCT의 COMPONENTS & PRODUCT-SPECIFIC ADJUSTMENT에서 "+ 사이즈별 사용량 추가"로 사이즈를 처음 지정하면, 더 이상 쓸모없어지는 "전체 (사이즈 미지정)" 행이 자동으로 함께 삭제됩니다. 이전엔 두 행이 같이 남아 헷갈렸습니다.
- 각 COMPONENT/재료의 예상원가를 눈에 잘 띄게 키웠습니다 — COMPONENT 전체 합계는 헤더에 크게, 사이즈별 예상원가는 각 사이즈 박스 안에 바로 보이게 옮겼습니다.
- 사이즈별 사용량 행을 각각 테두리 있는 박스로 구분해서, 어떤 입력이 어느 사이즈에 속하는지 한눈에 들어오게 했습니다. FORMULA VERSION 선택 박스 너비도 고정해서 레이아웃이 늘어지지 않게 정리했습니다.

**확인 방법**
- PRODUCT 상세 → COMPONENTS 섹션에서 사이즈별 사용량을 추가해보고 "전체 (사이즈 미지정)" 행이 자동으로 사라지는지, 예상원가가 크게 잘 보이는지 확인.

---

## 2026-09-24 — WORKFLOW 타임라인 시작/종료 표시 방식 개편 + TASK TYPE 색상 직접 지정

**무엇이 달라졌나**
- 타임라인의 TASK 블록이 소요 시간에 맞춰 통째로 칠해진 상자 대신, "시작 시각+이름 박스 → 색선 → 종료 시각 박스"로 바뀌었습니다. 소요 시간이 아주 짧은 TASK도 이제 이름과 TYPE이 항상 텍스트로 보입니다(이전엔 높이가 너무 눌려서 색선만 보이고 어떤 작업인지 안 보이는 경우가 있었습니다).
- WORKFLOW 탭 상단에 "TASK TYPE 색상 설정"이 생겼습니다. 펼치면 Mix/Cook/Bake 등 TASK TYPE별로 원하는 색을 팔레트에서 직접 골라 지정할 수 있고, 고른 색은 타임라인/TASK 목록/시간 미정 TASK 배지에 모두 반영됩니다. 지정하지 않은 TYPE은 이전처럼 이름 기반 자동 색상을 씁니다.

**확인 방법**
- PRODUCTION → 진행 중인 세션의 WORKFLOW 탭에서 타임라인 TASK 블록이 시작/종료 시각 두 줄 + 연결선 형태로 보이는지 확인.
- "TASK TYPE 색상 설정"을 펼쳐 원하는 TYPE의 색상 스와치를 눌러보고, 타임라인/TASK 목록에 바로 반영되는지 확인.
...
**무엇이 달라졌나**
- 체크(DONE)한 재료 칸이 회색 글씨만으로는 잘 안 보인다는 피드백으로, 배경에 음영을 넣고 가로선(취소선)도 더 굵게 넣어서 체크된 것과 안 된 것이 한눈에 구분되도록 했습니다.
- 계란 개수 같은 보조 계량("· 24개")이 무게 아래 줄에 따로 뜨던 것을, 같은 줄에 옆으로 나란히 표시되도록 바꿨습니다.

**확인 방법**
- WEIGHING MATRIX에서 재료를 체크해보고 회색 배경 + 굵은 가로선으로 눈에 띄게 바뀌는지, 계란처럼 보조 계량이 있는 재료가 "431g · 24개"처럼 한 줄로 표시되는지 확인.

---

## 2026-09-24 — WORKFLOW 실제 시작/완료 시각 직접 수정 + 타임라인이 실제 시각 반영

**무엇이 달라졌나**
- TASK EDIT 화면에 "실제 시작/완료" 시각 입력란이 추가됐습니다. 타임스탬프 버튼(상태 버튼) 누르는 걸 깜빡했거나 잘못 눌렀을 때, 직접 날짜/시간을 입력해서 바로잡을 수 있습니다.
- 타임라인이 계획 시각만 기준으로 그려지던 것을, 실제로 계획보다 일찍 시작/완료한 경우엔 그 실제 시각을 기준으로 위치가 조정되도록 고쳤습니다. 타임라인 창(4시간 범위)도 계획 시작 시각뿐 아니라 실제 시작 시각 중 더 이른 쪽을 기준으로 잡습니다 — 계획보다 일찍 시작한 TASK가 타임라인 창 밖으로 밀려나 안 보이던 문제도 함께 해결됩니다.

**확인 방법**
- 계획 시작 시각보다 이른 시각에 상태 버튼을 눌러 IN PROGRESS로 바꾼 뒤, 타임라인 창과 블록 위치가 그 실제 시각을 반영하는지 확인.
- TASK EDIT을 열어 "실제 시작/완료" 칸에 직접 시각을 입력하고 저장 → TASK LIST의 "실제" 줄에 반영되는지 확인.
