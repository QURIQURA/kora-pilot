...
function WorkSessionPage() {
  ...
  const weighingGroups = useMemo(
    ...
    [rows, ingredientsByVersion, progressByLineId],
  );
...
function WeighingView({
  ...
  return (
    <div className="space-y-4">
      {groups.map((group) => (
        <div key={group.ingredientId} className="border border-border">
          <div className="border-b border-border bg-secondary/40 px-3 py-2">
            <span className="label-caps text-sm">{group.ingredientName}</span>
          </div>
          <ul className="divide-y divide-border">
            {group.cells.map((cell) => (
              <WeighingCellRow
                key={cell.ingredientLineId}
                sessionId={sessionId}
                cell={cell}
...
function WeighingCellRow({
  ...
  return (
    <li className="flex min-h-[64px] flex-wrap items-center justify-between gap-3 px-3 py-3">
      <div className="flex items-center gap-3">
        <button
          type="button"
          className="label-caps flex min-h-[56px] min-w-[56px] items-center justify-center border border-foreground text-2xl"
          onClick={() => setProgress.mutate({ status: nextProgressStatus(cell.progressStatus) })}
          title={PROGRESS_STATUS_LABEL[cell.progressStatus]}
        >
          {PROGRESS_STATUS_ICON[cell.progressStatus]}
        </button>
        <div>
          <p className="text-base">{cell.formulaName}</p>
          <p className="label-caps text-xs text-muted-foreground">
            {PROGRESS_STATUS_LABEL[cell.progressStatus]}
          </p>
        </div>
      </div>
