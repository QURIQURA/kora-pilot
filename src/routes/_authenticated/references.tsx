...
import {
  ...
  ingredientsQuery,
  productsQuery,
  referenceEntriesQuery,
  techniqueCategoriesQuery,
} from "@/lib/queries";
import { leafTechniques, techniquePath } from "@/lib/technique";
import { isGeneralReference, REFERENCE_SOURCE_TYPES } from "@/lib/reference";
import { useSetBreadcrumb } from "@/components/layout/breadcrumb-context";
import { ReferenceCreateForm, ReferenceList } from "@/components/pilot/ReferenceSection";
...
export const Route = createFileRoute("/_authenticated/references")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "External references — books, videos, articles" },
      { property: "og:title", content: "PILOT — References" },
      { property: "og:description", content: "External references — books, videos, articles" },
...
type SourceFilter = "ALL" | (typeof REFERENCE_SOURCE_TYPES)[number];

const SCOPES: { key: Scope; label: string }[] = [
  { key: "ALL", label: "전체" },
  { key: "GENERAL", label: "일반 참고자료" },
...
function ReferencesPage() {
  useSetBreadcrumb([{ label: "PILOT", path: "/" }, { label: "REFERENCES" }]);

  const entries = useQuery(referenceEntriesQuery());
    ...
    return list.filter((entry) => {
      ...
      switch (scope) {
        case "ALL":
          return true;
        case "GENERAL":
          return isGeneralReference(entry);
        case "PRODUCT":
  ...
  }, [list, scope, targetId, sourceFilter]);
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border pb-4">
        ...
        <button type="button" className={buttonClass} onClick={() => setCreating((v) => !v)}>
          {creating ? "CLOSE" : "+ ADD REFERENCE"}
        </button>
      </div>

      {creating && <ReferenceCreateForm onDone={() => setCreating(false)} />}
      ...
      {entries.isLoading ? (
        ...
        <ReferenceList entries={filtered} />
      )}
    </div>
  );
}